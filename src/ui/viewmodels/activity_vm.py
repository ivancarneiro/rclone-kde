...
class ActivityViewModel(QObject):
    ...
    def activity_model(self):
        # MUY IMPORTANTE: Devolver una COPIA de la lista para que QML detecte el cambio de referencia
    ...
    def _process_log(self, task_id, line):
        ...
        if "Deleted" in clean_line:
            ...
            if filename: changed = self._add_or_update_item(filename, 0, 0, "deleted", is_active=False)
        elif any(x in clean_line for x in ["Copied", "Updated", "Moved", "Synchronized"]):
            filename = self._extract_filename(clean_line)
            if filename: changed = self._add_or_update_item(filename, 0, 0, "success", is_active=False)
        elif "Failed to" in clean_line or "ERROR" in clean_line:
            filename = self._extract_filename(clean_line)
            if filename: changed = self._add_or_update_item(filename, 0, 0, "error", is_active=False)
        if changed:
            self.activityChanged.emit()
